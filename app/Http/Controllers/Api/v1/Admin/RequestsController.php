...
class RequestsController extends Controller
{
    ...
    public function index(HttpRequest $request)
    {
        ...
        if (empty($selectedConf)) {

            $conferences = Conference::all();
            foreach ($conferences as $item) {
                array_push($selectedConf, $item->id);
            }
        }
        ...
        $approved = Request::where([['status', '=', Request::STATUS_APPROVED]])->whereIn('conference_id', $selectedConf)->whereIn('category_id', $catIds)->count();
        $completed = Request::where([['status', '=', Request::STATUS_COMPLETED]])->whereIn('conference_id', $selectedConf)->whereIn('category_id', $catIds)->count();
        $not_approved = Request::where([['status', '=', Request::STATUS_NOT_APPROVED]])->whereIn('conference_id', $selectedConf)->whereIn('category_id', $catIds)->count();
        $fail = Request::where([['status', '=', Request::STATUS_FAIL]])->whereIn('conference_id', $selectedConf)->whereIn('category_id', $catIds)->count();
        $re_upload = Request::where([['status', '=', Request::STATUS_RE_UPLOAD]])->whereIn('conference_id', $selectedConf)->whereIn('category_id', $catIds)->count();
        $paid = Request::where([['payment_status', '=', Request::PAYMENT_STATUS_PAID]])->whereIn('conference_id', $selectedConf)->whereIn('category_id', $catIds)->count();
        $un_paid = Request::where([['payment_status', '=', Request::PAYMENT_STATUS_UNPAID]])->whereIn('conference_id', $selectedConf)->whereIn('category_id', $catIds)->count();
        $sent = Request::where([['payment_status', '=', Request::PAYMENT_STATUS_SENT]])->whereIn('conference_id', $selectedConf)->whereIn('category_id', $catIds)->count();
